...
if($filename[1] == 'csv'){
    ...
    
    {
      ...
      if(!empty($result)){
        $query = "UPDATE tbl_grades SET prelim = '$item5', midterm = '$item6', prefinal = '$item7', final = '$item8' WHERE s_id = '$item1' AND sbj_id = '$item2' AND sec_id = '$secid'";
      }else{
        $query = "INSERT INTO tbl_grades (s_id, sbj_id, sy_id, sem_id, sec_id, yr_id, prelim, midterm, prefinal, final) 
        VALUES('$item1', '$item2', '$item3','$item4','$secid', '$yr', '$item5','$item6','$item7','$item8')";
